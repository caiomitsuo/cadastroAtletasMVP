...
import { columnsViagem } from './tableViagem'
  ...
  setup() {
    ...
    return {
      ...
      exportTable () {
        ...
        const content = [columnsViagem.map(col => wrapCsvValue(col.label))].concat(
          filteredRows.map(row => columnsViagem.map(col => wrapCsvValue(
        ...
        if (status !== true) {
          $q.notify({
            message: 'Navegador não autorizou o download',
            color: 'negative',
            icon: 'warning'
          })
        }
